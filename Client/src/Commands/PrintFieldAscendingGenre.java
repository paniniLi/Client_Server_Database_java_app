...
public class PrintFieldAscendingGenre extends Command implements Serializable {
    ...
    public void setParamFromFile(ArrayList<String> paramString) throws InvalidParams {
        if (paramString.size() != 1 || paramString.get(0).split(" ").length != 1) throw new InvalidParams(ERROR_MESSAGE+"Invalid data for command printFieldAscendingGenre."+ ERROR_MESSAGE);
    }
}
